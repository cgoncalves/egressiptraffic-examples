...
src=$(kubectl exec -n demo-eipt demo-pod -- curl -s --connect-timeout 5 http://192.168.250.100:8080 2>/dev/null | grep -oP 'client=\K[0-9.]+')
if [ "$src" = "192.168.150.101" ]; then
    PASS "Traffic to destination network uses EgressIP (source: $src)"
else
