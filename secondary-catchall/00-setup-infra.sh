#!/bin/bash
# Sets up infrastructure for the secondary-catchall example:
# A catch-all EgressIP on a secondary host interface routes ALL
# pod traffic through the secondary interface.
#
# Network layout:
#
...
#                                         +------------------------------+
#
# eip-default (catch-all, no trafficSelector):
#   192.168.150.101 on oam-link-net — handles ALL traffic from the pod
...
echo "=== Done ==="
echo "Link network:   192.168.150.0/24 (Docker)"
echo "Router:         192.168.150.1 (oam-router)"
echo "Dest network:   192.168.250.0/24 (server: 192.168.250.100:8080)"
echo "EgressIP:       192.168.150.101 (catch-all)"
