#!/bin/bash
# Sets up secondary network infrastructure on ovn-worker2 for L4 filtering demo.
# Two services run in the oam netns on different ports:
#   - SIP signaling on UDP 5060
#   - SSH management on TCP 22
#
...
#   |   via 192.168.150.1      |              | (IP forwarding enabled)    |
#   +--------------------------+              | UDP listener :5060 (SIP)   |
#                                             | TCP listener :22 (SSH)     |
#                                             | HTTP listener :8080 (all)  |
#                                             +----------------------------+
...
for node in ovn-worker ovn-worker2; do
    docker exec "${node}" bash -c "apt-get update -qq && apt-get install -y -qq tcpdump curl python3 netcat-openbsd socat > /dev/null 2>&1"
done
...
# Write HTTP listener script (for verifying source IP)
...
# UDP echo server on port 5060 (simulating SIP signaling)
docker exec "${EGRESS_NODE}" bash -c "nsenter --net=/var/run/netns/oam pkill -f 'socat.*UDP-LISTEN:5060' 2>/dev/null" || true
docker exec -d "${EGRESS_NODE}" nsenter --net=/var/run/netns/oam socat -v UDP-LISTEN:5060,fork SYSTEM:'echo "SIP-OK client=\$SOCAT_PEERADDR"'

# TCP echo server on port 22 (simulating SSH management)
docker exec "${EGRESS_NODE}" bash -c "nsenter --net=/var/run/netns/oam pkill -f 'socat.*TCP-LISTEN:2222' 2>/dev/null" || true
docker exec -d "${EGRESS_NODE}" nsenter --net=/var/run/netns/oam socat -v TCP-LISTEN:2222,fork,reuseaddr SYSTEM:'echo "SSH-OK client=\$SOCAT_PEERADDR"'

sleep 1
docker exec "${EGRESS_NODE}" ip netns exec oam ss -tlnp | grep -q ":8080" || echo "WARNING: HTTP listener on port 8080 failed to start"
docker exec "${EGRESS_NODE}" ip netns exec oam ss -ulnp | grep -q ":5060" || echo "WARNING: UDP listener on port 5060 failed to start"
docker exec "${EGRESS_NODE}" ip netns exec oam ss -tlnp | grep -q ":2222" || echo "WARNING: TCP listener on port 2222 failed to start"

echo "=== Done ==="
echo "HTTP listener:  192.168.250.1:8080 (all traffic, for source IP verification)"
echo "SIP signaling:  192.168.250.1:5060/UDP"
echo "SSH management: 192.168.250.1:2222/TCP"
