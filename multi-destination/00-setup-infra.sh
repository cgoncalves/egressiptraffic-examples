...
# netns-based destination server that has two IPs.
#
# Network layout:
#
...
docker run -d --name oam-router --network oam-link-net --ip 192.168.150.1 \
    ...
    bash -c '
        ...
        ip netns exec oam-server ip addr add 192.168.250.200/24 dev veth-ns
        ...
        # Start HTTP listeners in server netns
...
PYEOF
        ...
        nsenter --net=/var/run/netns/oam-server python3 /tmp/http-listener.py 8081 &
...
sleep 3
echo "=== Verifying ==="
docker exec oam-router curl -s --connect-timeout 2 http://192.168.250.100:8080 || echo "WARNING: router cannot reach server1"
docker exec oam-router curl -s --connect-timeout 2 http://192.168.250.200:8081 || echo "WARNING: router cannot reach server2"
...
echo "Dest network:   192.168.250.0/24 (netns inside oam-router, server1: 192.168.250.100:8080, server2: 192.168.250.200:8081)"
