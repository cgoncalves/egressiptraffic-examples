...
A router container bridges the link and destination networks, simulating a real deployment where the external server is behind a gateway. This demonstrates the priority-based coexistence: destination-specific rules (priority 100) are evaluated before the catch-all (priority 99).
