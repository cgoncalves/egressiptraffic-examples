#!/bin/bash
# Verify on-link EgressIPTraffic: destination on the same L2 as EgressIP.
set -euo pipefail
...
echo "=== Test 1: Traffic to 192.168.150.200 (on-link, same L2 as EgressIP) ==="
src=$(kubectl exec -n demo-eipt demo-pod -- curl -s --connect-timeout 5 http://192.168.150.200:8080 2>/dev/null | grep -oP 'client=\K[0-9.]+')
if [ "$src" = "192.168.150.101" ]; then
    PASS "On-link traffic uses EgressIP (source: $src)"
else
